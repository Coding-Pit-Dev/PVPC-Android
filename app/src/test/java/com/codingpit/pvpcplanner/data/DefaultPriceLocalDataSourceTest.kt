...
class DefaultPriceLocalDataSourceTest {
    ...
    @Test
    fun `getPrices converts ISO date to dd-MM-yyyy and maps entities`() = runTest {
        val entities = listOf(
    ...
    @Test
    fun `getPrices returns empty list for invalid date format`() = runTest {
        val result = dataSource.getPrices("not-a-date")
        assertTrue(result.isEmpty())
    }

    @Test
    fun `getPrices returns empty list when DAO returns nothing`() = runTest {
    ...
    @Test
    fun `savePrices inserts mapped entities into DAO`() = runTest {
        val prices = listOf(
