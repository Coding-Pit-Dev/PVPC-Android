...
object DeviceTestFixtures {
    ...
    object UpdateScenarios {
        ...
        fun updateDeviceHours(
            device: Device,
            newHours: Int,
        ) = device.copy(hours = newHours)

        fun updateDeviceIcon(
            device: Device,
            newIcon: String,
        ) = device.copy(icon = newIcon)

        val WASHING_MACHINE_UPDATED_NAME =
            updateDeviceName(
                CommonDevices.WASHING_MACHINE,
                "Samsung EcoBubble Washing Machine",
            )

        val DISHWASHER_EFFICIENCY_UPGRADE =
            updateDeviceHours(
                CommonDevices.DISHWASHER,
                3, // Upgraded to more thorough cycle
            )

        val DRYER_ICON_UPDATE =
            updateDeviceIcon(
                CommonDevices.DRYER,
                "dryer_modern",
            )
