...
class StatsViewModelTest {
    ...
    @Test
    fun `emits Success with summaries when history loads`() = runTest {
    ...
    @Test
    fun `emits Success with empty list when no history`() = runTest {
        coEvery { mockGetPriceHistory() } returns Result.success(emptyList())
        viewModel = StatsViewModel(mockGetPriceHistory, mockErrorHandler, testDispatcher)
    ...
    @Test
    fun `summaries expose correct avg min max per day`() = runTest {
        val summaries = listOf(
