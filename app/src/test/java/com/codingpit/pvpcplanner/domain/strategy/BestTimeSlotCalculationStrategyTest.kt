...
class BestTimeSlotCalculationStrategyTest {
    ...
    @Test
    fun `calculateBestTimeSlot returns first slot when all prices are equal`() = runTest {
        // Arrange
    ...
    @Test
    fun `calculateBestTimeSlot finds cheapest consecutive slot for 1-hour device`() = runTest {
        // Arrange
    ...
    @Test
    fun `calculateBestTimeSlot finds cheapest consecutive slot for multi-hour device`() = runTest {
        // Arrange
    ...
    @Test
    fun `calculateBestTimeSlot handles minimum duration device with single price`() = runTest {
        // Arrange
        val device = Device(1, "Test Device", 1, "test_icon")
        val prices = listOf(
            PVPCModel("2023-10-15", 0, 1, 0.15, 0.18)
