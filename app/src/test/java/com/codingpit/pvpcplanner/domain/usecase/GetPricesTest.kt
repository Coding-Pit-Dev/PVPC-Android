...
class GetPricesTest {
    ...
    @Test
    fun `invoke with specific date returns prices from repository`() = runTest {
        // Arrange
        val date = "2023-10-15"
    ...
    @Test
    fun `invoke with empty date uses current date`() = runTest {
        // Arrange
        val expectedPrices = listOf(
            PVPCModel("2023-10-15", 0, 1, 0.15, 0.18)
        )
    ...
    @Test
    fun `invoke with no parameters uses current date`() = runTest {
        // Arrange
        val expectedPrices = listOf(
            PVPCModel("2023-10-15", 0, 1, 0.15, 0.18)
        )
