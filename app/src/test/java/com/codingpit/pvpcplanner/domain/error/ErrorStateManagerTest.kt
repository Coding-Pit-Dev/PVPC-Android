...
class ErrorStateManagerTest {
    data class TestState(
        ...
        companion object Factory : HasErrorState<TestState> {
            override fun createErrorState(errorResult: ErrorResult): TestState = TestState("ERROR: ${errorResult.message}")
        }
    }
    ...
    @Test
    fun `handleErrors extension function catches exceptions and creates error state`() =
        runTest {
            // Arrange
            val testFlow =
                flowOf(TestState("test"))
    ...
    @Test
    fun `handleErrors creates error state when exception occurs`() =
        runTest {
            // Arrange
            val errorFlow =
                flow<TestState> { throw RuntimeException("Test error") }
                    .handleErrors(errorHandler, "test_context", TestState.Factory)
    ...
    @Test
    fun `handleResultErrors catches exceptions and emits Result failure`() =
        runTest {
            // Arrange
            val errorFlow =
                flow<Result<String>> { throw RuntimeException("Test error") }
    ...
    @Test
    fun `handleResultErrors preserves successful results`() =
        runTest {
            // Arrange
            val successFlow =
